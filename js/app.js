...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const secciones = ['inicio', 'acerca', 'oferta', 'admision', 'contactos'];
  ...
  // Enlaces del menú principal (header y drawer)
  const navLinks = Array.from(document.querySelectorAll('.mdl-navigation__link'));
  navLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      // Buscar el texto del enlace para identificar la sección
      const texto = link.textContent.trim().toLowerCase();
      let id = '';
      if (texto.includes('inicio')) id = 'inicio';
      else if (texto.includes('acerca')) id = 'acerca';
      else if (texto.includes('oferta')) id = 'oferta';
      else if (texto.includes('admisión')) id = 'admision';
      else if (texto.includes('contacto')) id = 'contactos';
      if (id) mostrarSeccion(id);
      // Cerrar el drawer en móviles
